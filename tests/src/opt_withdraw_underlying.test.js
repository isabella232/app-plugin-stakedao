...
test('[Nano S] Deposit underlying into options', zemu("nanos", async (sim, eth) => {
  ...
  const {data} = await contract.populateTransaction.withdrawUnderlying(AMOUNT_TO_WITHDRAW, MIN_UNDERLYING);
  ...
  await sim.navigateAndCompareSnapshots('.', 'nanos_opt_withdraw_under', [BASE_SCREENS_S, 0]);
  await tx;
}));
...
test('[Nano X] Deposit underlying into options', zemu("nanox", async (sim, eth) => {
  ...
  await waitForAppScreen(sim);
  await sim.navigateAndCompareSnapshots('.', 'nanox_opt_withdraw_under', [BASE_SCREENS_X, 0]);
  await tx;
}));