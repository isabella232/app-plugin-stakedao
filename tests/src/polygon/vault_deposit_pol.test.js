...
test('[Nano S] Deposit Tokens into vault', zemu("nanos", async (sim, eth) => {
  ...
  await sim.navigateAndCompareSnapshots('.', 'nanos_vault_deposit_pol', [BASE_SCREENS_S, 0]);
  await tx;
}, NETWORK));
...
//   await waitForAppScreen(sim);
//   await sim.navigateAndCompareSnapshots('.', 'nanox_vault_deposit_pol', [BASE_SCREENS_X, 0]);
//   await tx;
// }, NETWORK));